...
export async function runGreedySchedule({
  onProgress,
  delayMs = 300,
}: {
  ...
  update("מריץ אלגוריתם גרידי...", 55);
  const solveRes = await fetch("/api/greedy-schedule", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ teams, slots }),
  });

  if (!solveRes.ok) throw new Error("שגיאה בהרצת האלגוריתם הגרידי");
  ...
  update("השיבוץ הגרידי הושלם בהצלחה!", 100);
}
